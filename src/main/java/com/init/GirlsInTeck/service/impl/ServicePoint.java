...
public class ServicePoint {
	...
	public String deletePointById(int id) {
		...
		if (!point.isEmpty()) {
			...
			int idAddress=pointForDelete.getAddress().getIdAddress();
			int idLocation=pointForDelete.getLocation().getId_location();
	...
	public String updatePoint(int id, PointDto PointDto) {
		...
		if (!Optionalpoint.isEmpty()) {
			...
			System.out.println(updatepoint.getAddress().getIdAddress());
			updatepoint.setActivity(PointDto.getActivity());
			updatepoint.setColor(PointDto.getColor());
			
//			// update address and save
			Address address = addressRepository.findById(updatepoint.getAddress().getIdAddress()).get();
			...
			serviceAddress.updateAddress(address.getIdAddress(), addressDto);
	...
	public List<Point> findByColor(String color) {
		
		List<Point> points =pointRepository.findAll();
		List<Point> pointsColor = new ArrayList<Point>();
		for(Point point:points) {
			if(point.getColor().getColorName().equals(color)) {
				pointsColor.add(point);
			}
		}
		return pointsColor;
	}
